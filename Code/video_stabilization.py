...
def crop_image(frame: np.ndarray) -> np.ndarray:
    slice_x, slice_y = ndimage.find_objects(frame > 0)[0]
    return frame[slice_x, slice_y]
...
def stabilize_video(
    ...
    while cap.isOpened():
        ...
        crop_ind = int(window_size + project_constants.CROP_BORDER_EXTRA)
        if not isFirstFrame:
            (b, g, r) = cv2.split(frame)
            b = warp_image(b, u, v)
            b = crop_image(b[crop_ind:h - crop_ind, crop_ind:w - crop_ind])
            if b.shape != size:
                b = cv2.resize(b, size)
            g = warp_image(g, u, v)
            g = crop_image(g[crop_ind:h - crop_ind, crop_ind:w - crop_ind])
            if g.shape != size:
                g = cv2.resize(g, size)
            r = warp_image(r, u, v)
            r = crop_image(r[crop_ind:h - crop_ind, crop_ind:w - crop_ind])
            if r.shape != size:
                r = cv2.resize(r, size)
